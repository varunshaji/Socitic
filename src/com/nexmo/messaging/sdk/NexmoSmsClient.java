...
public class NexmoSmsClient {
     ...
     * @throws Exception There has been a general failure either within the Client class, or whilst attempting to communicate with the Nexmo service (eg, Network failure)
     */
 /*bbbbbb   public SmsSubmissionResult[] submitMessageHttps(Message message) throws Exception {
        boolean https = true;
        return submitMessage(message, https);
    }             bbbbbbbbbbb*/
...
//bbb   for (int pass=1;pass<=2;pass++) {
            ...
            try {
            	...
            	HTTPRequest httpRequest = new HTTPRequest(url2 , HTTPMethod.GET); 
            	response = urlFetchService.fetch(httpRequest); 
  /*sms snd          	httpRequest = new HTTPRequest(url2 , HTTPMethod.GET); 
            	response = urlFetchService.fetch(httpRequest); */
            /*	if (this.httpClient == null)
                    this.httpClient = HttpClientUtils.getInstance(this.connectionTimeout, this.soTimeout).getNewHttpClient(); */     
                int status = response.getResponseCode();//this.httpClient.executeMethod(method);
                if (status != 200)
     ...
     //   Document doc = null;
        /*synchronized(this.documentBuilder) {
            try {
                ...
                doc = this.documentBuilder.parse(new InputSource(new StringReader(response1)));
            } catch (Exception e) {
